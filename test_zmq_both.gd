...
@onready var zmq_receiver: ZMQReceiver = ZMQReceiver.new_from("tcp://localhost:9991", ZMQ.SocketType.PULL, ZMQ.ConnectionMode.BIND, "")
@onready var zmq_sender: ZMQSender = ZMQSender.new_from("tcp://localhost:9991", ZMQ.SocketType.PUSH, ZMQ.ConnectionMode.CONNECT, "", false)
